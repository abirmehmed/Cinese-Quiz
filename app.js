...
'use strict';
  ...
  // ── HSK helpers ────────────────────────────────────────────
  function hskWords(key){
    if(key === '1-3') return [...HSK_DATA.hsk1,...HSK_DATA.hsk2,...HSK_DATA.hsk3];
    if(key === '1-6') return [1,2,3,4,5,6].flatMap(n=>HSK_DATA['hsk'+n]);
    if(/^[1-6]$/.test(key)) return HSK_DATA['hsk'+key];
    return [];
  }
  ...
  document.querySelectorAll('[data-hsk]').forEach(btn=>{
    btn.addEventListener('click',()=>{
      document.querySelectorAll('[data-hsk]').forEach(b=>b.classList.remove('selected'));
      btn.classList.add('selected');
      selectedHsk = btn.dataset.hsk;
      rebuildDataset();
    });
  });
  ...
  function rebuildDataset(){
    ...
    if(raw){
      raw.split('\n').forEach((line,i)=>{
        ...
        customWords.push({han:f[0],pinyin:f[1]||'',meaning:f[2]});
      });
    }

    let base = [];
    if(selectedHsk && selectedHsk!=='custom'){
      base = hskWords(selectedHsk).map(([h,p,m])=>({han:h,pinyin:p,meaning:m}));
    }
    fullset = selectedHsk==='custom' ? customWords : [...base,...customWords];
  ...
  function renderList(query){
    ...
    tbody.innerHTML='';
    if(!fullset.length){
      tbody.innerHTML='<tr><td colspan="4" class="list-empty">No words loaded — pick an HSK level above.</td></tr>';
    ...
    fullset.forEach((w,i)=>{
      const num = i+1;
      if(query){
        const hay = (w.han+' '+w.pinyin+' '+w.meaning).toLowerCase();
      ...
      tr.innerHTML=`
        <td class="num">${num}</td>
        <td class="han">${esc(w.han)}</td>
        <td class="py">${esc(w.pinyin)}</td>
  ...
  function directionFor(){
    ...
    return Math.random()<.5 ? 'meaning' : 'hanzi';
  }
  ...
  function nextCard(){
    ...
    if(direction==='meaning'){
      ...
      promptMain.textContent  = word.han;
      promptMain.classList.remove('meaning-mode');
      promptSub.textContent   = word.pinyin||'';
    } else {
      promptLabel.textContent = 'Which character matches this meaning?';
    ...
    allPos.forEach(optPos=>{
      ...
      if(direction==='meaning'){
        ...
        btn.innerHTML=`<span class="opt-han">${esc(w.han)}</span><span style="color:#9a9182;font-size:12px;">${esc(w.pinyin||'')}</span>`;
      }
  ...
  function finishRound(){
    ...
    if(!missedSet.size){
      ...
      missedSet.forEach(w=>{
        ...
        row.innerHTML=`<span><span class="han">${esc(w.han)}</span><span class="py">${esc(w.pinyin||'')}</span></span><span class="mean">${esc(w.meaning)}</span>`;
        mc.appendChild(row);
      });
    }
